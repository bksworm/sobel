...
const suiteBenchmarkMethodPrefix = "Benchmark"
...
Run sets up the suite, runs its test cases and tears it down:
    ...
    2. Seeks for any methods that have `Benchmark` prefix, for each of them it:
      a. Calls `SetUp`
      b. Calls the test method itself
      c. Calls `TearDown`
    3. Calls `suite.TearDownSuite`
*/
func Bench(b *testing.B, suite TestSuite) {
	suite.SetUpSuite()
	...
	for i := 0; i < suiteType.NumMethod(); i++ {
		...
		if strings.HasPrefix(m.Name, suiteBenchmarkMethodPrefix) {
			b.Run(m.Name, func(b *testing.B) {
				suite.SetUp()
				defer suite.TearDown()

				in := []reflect.Value{reflect.ValueOf(suite), reflect.ValueOf(b)}
				m.Func.Call(in)
			})
		}
	}
